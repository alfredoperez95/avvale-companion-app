...
7. **CSP (anti-inyección)**: el frontend genera nonce por request (`frontend/src/middleware.ts`) y por defecto envía la política estricta en `Content-Security-Policy-Report-Only`. Para pasar a bloqueo real, define `CSP_ENFORCE=true` en producción y redeploya. Antes/después del cambio revisa logs de `POST /api/csp-report` y smoke-test: login magic-link, launcher, gastos (preview PDF), MEDDPICC + Convai, KYC React, editor rich text / plantillas email e iconos Fiori. La política no permite `unsafe-inline` ni `unsafe-eval` en prod, bloquea `object-src`, limita `frame-src` a `blob:` + ElevenLabs y mantiene HSTS/COOP/CORP. El backend exige `JWT_SECRET`/`MAGIC_LINK_SECRET` fuertes y `CORS_ORIGIN` al arrancar en producción, Helmet reforzado y **rate limit global** (más estricto en auth/webhooks públicos). Tras un incidente en el host, **rota secretos** y haz **rebuild** desde git limpio.
8. **Headers HTTP**: comprueba con `curl -I` que HTML, assets, KYC legacy, endpoint CSP y API devuelven `Strict-Transport-Security`, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `Cross-Origin-Embedder-Policy`, `Cross-Origin-Opener-Policy` y `Cross-Origin-Resource-Policy`. Rutas sugeridas: `/login`, `/fonts/sap/72-Regular.woff2`, `/kyc/index.html`, `POST /api/csp-report` y `/health` o `/api/health` según el enrutado real del proxy.
9. **Cookies**: la web no debe crear cookies desde cliente. La antigua cookie `appearance` se elimina y la preferencia de tema se guarda en `localStorage`; cualquier cookie futura debe emitirse server-side con `Secure`, `HttpOnly` y `SameSite=Lax` o `Strict`. En este alcance la autenticación sigue usando `Authorization: Bearer`; migrar el JWT a cookie `HttpOnly` requiere una fase específica con logout server-side y adaptación CORS.
10. **Subida de archivos**: el backend valida extensión, MIME declarado, firma real, tamaño y estructura básica antes de persistir. Todos los nombres se sustituyen por UUID; el nombre original solo se conserva como display name saneado. Se bloquean siempre SVG, HTML, JS, PHP, ejecutables y dobles extensiones peligrosas. Límites actuales: avatar 2 MB; gastos/Yubiq/activaciones 20 MB; MEDDPICC 25 MB; RFQ 50 MB. Las descargas usan `attachment`/`nosniff` donde aplica y las rutas de disco deben resolverse con guard contra path traversal.
11. **Sinks DOM**: no usar `dangerouslySetInnerHTML` ni `innerHTML` con datos sin sanitizar/escapar. En React, usar `sanitizeUserHtml` o formatters que devuelvan HTML ya sanitizado. En HTML legacy, todo dato de API debe pasar por `escapeHtml`/`escapeAttr` y los enlaces externos deben normalizarse a `http/https` con `rel="noopener noreferrer"`. `eval` y `new Function` están prohibidos.
