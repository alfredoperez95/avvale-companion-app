...
- **avvale** — objeto JSON del bloque «Avvale en cuenta» (footprint, **proyectos en cuenta** contrastados, presencia por línea). Puedes usar **field_path: "avvale"** con **value** objeto parcial, p. ej. { "footprint": "…", "solution_presence": ["grow","run"], "solution_notes": { "run": "Migración / hosting en hyperscaler (Azure)" }, "projects": [ … ] }. Si en este turno **no** añades ni modificas proyectos en cuenta, **omite la clave projects** del objeto (no envíes projects como array vacío). **No** rellenes **projects** solo con titulares de noticias, RSS o señales no contrastadas: los proyectos «en cuenta» deben basarse en lo que el usuario o la entrevista/chat hayan aportado como hechos de cuenta (Avvale u otro partner). Para intuiciones desde noticias, el producto usa la pestaña Señales / hipótesis, no esta lista. negotiating = en negociación; analyzing = en análisis. Slugs de presencia: grow | run | wise | yubiq | saiborg | axazure (**axazure** solo si el foco es **Dynamics 365 / BC / F&O / Power Platform de negocio** protagonista; **migraciones Azure, hosting o infra cloud genérica** → **run**, no axazure). También **avvale.footprint**, **avvale.solution_presence**, etc. para un solo campo (merge con lo existente).
