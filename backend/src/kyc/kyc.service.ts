...
Entre 1 y 8 elementos en "hypotheses". No dupliques títulos casi idénticos.`;
  ...
  private async recordKycAudit(prismaLike: unknown, entry: KycAuditEntry): Promise<void> {
    ...
    const createdKycAudit = (await writer.kycAuditLog.create({ data })) as { id?: bigint | number | string };
    try {
      await writer.auditLog?.create({
        data: {
          ...
          meta: toAuditJson({
            ...
            kycAuditLogId: createdKycAudit.id == null ? null : String(createdKycAudit.id),
